...
#property link      "https://trading.bdev.com"
#property version   "2.00"
#property indicator_chart_window
#property indicator_plots 0
#property description "Superpone velas HTF en tu grafico. 1 clic para elegir TF, otro para ocultar."
#property description "Overlays HTF candles on your chart. Click to pick a TF, click again to hide."
...
const string SETTINGS_FILE = "MultiTF_settings.txt";
...
int OnInit()
{
   ...
   // Aplica el estado restaurado
   UpdatePanelVisibility();
   if(g_selected_tf_index >= 0)
   {
      HighlightButton(g_selected_tf_index, true);
      RedrawAllCandles();
   }
   UpdateToggleText();
                  ...
                  const string &sparam)
{
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      ...
      {
         string tf_name = StringSubstr(sparam, StringLen(PREFIX_BTN));
         ...
         {
            HighlightButton(tf_index, false);
            g_selected_tf = WRONG_VALUE;
            g_selected_tf_index = -1;
            ClearCandles();
         }
         else
         {
            ...
            HighlightButton(old_index, false);
            HighlightButton(tf_index, true);
...
void HighlightButton(int index, bool selected)
{
   if(index < 0) return;
   string btn_name = PREFIX_BTN + TFNames[index];
   if(ObjectFind(0, btn_name) < 0) return;

   if(selected)
   {
      ObjectSetInteger(0, btn_name, OBJPROP_BGCOLOR, clrDarkOrange);
...
void UpdateToggleText()
{
   ...
   if(ObjectFind(0, toggle_name) < 0) return;

   if(g_selected_tf_index >= 0)
      ObjectSetString(0, toggle_name, OBJPROP_TEXT, TFNames[g_selected_tf_index]);
   else
      ObjectSetString(0, toggle_name, OBJPROP_TEXT, "TF");
}

//+------------------------------------------------------------------+
//| Guarda TF seleccionado y visibilidad del panel                    |
//+------------------------------------------------------------------+
void SaveSettings()
{
   ...
   FileWrite(handle, g_selected_tf_index, g_panel_visible);

   FileClose(handle);
}

//+------------------------------------------------------------------+
//| Restaura TF seleccionado y visibilidad del panel                  |
//+------------------------------------------------------------------+
void LoadSettings()
{
   ...
   g_selected_tf_index = (int)FileReadNumber(handle);
   g_panel_visible = (bool)FileReadNumber(handle);

   // Valida que el indice siga siendo valido
   if(g_selected_tf_index < 0 || g_selected_tf_index >= ArraySize(Timeframes))
   {
      g_selected_tf_index = -1;
      g_selected_tf = WRONG_VALUE;
   }
   ...
   {
      g_selected_tf = Timeframes[g_selected_tf_index];
      // Si el TF ya no es superior al actual, invalida
      if(g_selected_tf <= Period())
      {
         g_selected_tf = WRONG_VALUE;
         g_selected_tf_index = -1;
...
void CreateTFButtons()
{
   ...
   for(int i = 0; i < total; i++)
   {
      ...
      HighlightButton(i, (g_selected_tf_index == i));
...
void RepositionUI()
{
   ...
   if(ObjectFind(0, toggle_name) >= 0)
   {
      ObjectSetInteger(0, toggle_name, OBJPROP_XDISTANCE, start_x);
      ObjectSetInteger(0, toggle_name, OBJPROP_YDISTANCE, PanelY);
   }
