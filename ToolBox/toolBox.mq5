...
// Si ya existen, elimínalos para recrear correctamente
  ...
  for(int i=0; i<ArraySize(buttons); i++)
   {
    ...
    if(!ObjectCreate(0, name, OBJ_BUTTON, 0, 0, 0))
     {
      // si falla la creación, intentar borrar y recrear una vez
      ObjectDelete(0, name);
...
// --- Recentrar si se cambia el tamaño del gráfico ---
  if(id == CHARTEVENT_CHART_CHANGE)
   {
    // Solo recrea si la toolbox está visible
    if(SwitchToolbox)
      CreatePanel();
    ChartRedraw();
   }

// --- Clic en el gráfico (para dibujar) ---
  if(id == CHARTEVENT_CLICK && active_tool != "" && wait_for_click)
   {
    int x = (int)lparam;
    int y = (int)dparam;

    // Ignorar clics dentro del área del panel (si visible)
    if(SwitchToolbox)
     {
      int chart_width = (int)ChartGetInteger(0, CHART_WIDTH_IN_PIXELS);
      int total_width = ArraySize(buttons) * (BTN_SIZE + 5);
    ...
    // Desactivar herramienta después de dibujar y "soltar" visualmente los botones
    active_tool = "";
    wait_for_click = false;
...
//+------------------------------------------------------------------+
//| Crear objeto según la herramienta seleccionada                  |
//+------------------------------------------------------------------+
...
// Si la creación falló, evitamos hacer set de propiedades en nombre inválido
...
//+------------------------------------------------------------------+
//| Restaurar color de botón según tipo                             |
//+------------------------------------------------------------------+
